...
if sys.version_info >= (3, 0):
	...
	import fontParts.world as world

else:
	from itertools import izip
	import robofab.world as world
...
class Autokern(TFSMap):
	...
	def configureLogging(self):
		...
		print(TFSProject.findProjectRootFolder())
		print(TFSProject.findProjectRootFolder())
		print(TFSProject.findProjectRootFolder())
		print(TFSProject.findProjectRootFolder())
		print(TFSProject.findProjectRootFolder())
...
#            print 'self.glyphs_to_kern', self.glyphs_to_kern
		...
		def isValidGlyphCategories(key, value):
			if sys.version_info >= (3, 0): 
				...
				instance_check = type(value).__name__ not in ['str', 'unicode']

			else:

				instance_check = type(value) not in ( types.StringType, types.UnicodeType, )
...
#                ( 'x-extrema Overlap Scaling', 'x_extrema_overlap_scaling',),
		...
		for key in sorted(self.argumentsMap):
			...
			if sys.version_info >= (3, 0): 
				val_check = isinstance(type(value), (bool))
				val_check_b = isinstance(type(value), (list, tuple))
			else:
				val_check = (type(value) == types.BooleanType)
				val_check_b = type(value) in ( types.ListType, types.TupleType, )
			if val_check:
				if value:
	...
	def makeDefaultMustacheMap(self, localsMap=None):
		...
		def addToMustacheMap(key, value):
			...
			if sys.version_info >= (3, 0): 
				...
				val_check = not isinstance(type(value), (int,float,str))
				val_check_b =  isinstance(type(value), (int,float))

			else:

				import types
				val_check = type(value) not in ( types.IntType,
									types.LongType,
									types.FloatType,
									types.UnicodeType,
									types.StringType, )
				val_check_b = type(value) in ( types.IntType,
									types.LongType)
...
#            addToMustacheMap(attr, value)
		...
		for key, value in self.items():
			...
			if sys.version_info >= (3, 0): 
				...
				val_check = isinstance(value, (int,float)) # no long-er

			else:

				import types
				val_check = type(value) in ( types.IntType,	types.FloatType, types.LongType,)
		...
		if localsMap:
			for key, value in localsMap.items():
				...
				if sys.version_info >= (3, 0): 
					val_check = isinstance(value, (int,float))
				else:
					val_check = type(value) in ( types.IntType,
									types.FloatType,
									types.LongType,)
				if val_check:
					emsKey = key + '_in_ems'
...
#        print 'intrusionTotal', intrusionTotal, 'protrusionTotal', protrusionTotal, 'intrusionToleranceArea', intrusionToleranceArea
	...
	def findMinProfileAdvance(self, profile0, profile1):
		...
		for edge0, edge1 in izip(profile0, profile1):
...
#        glyphMinmax1 = minmaxPaths(glyphContours1)
		...
		def fillGlyphContours(glyphContours, glyphColor):
			result = ()
			tempUfoFont = world.NewFont(familyName='ignore', styleName='ignore')
