        ...
        try:
            if not onJython:
                if sys.version_info >= (3, 0): 
                    ...
                    instance_check = not isinstance(self.points, (list,tuple))

                else:

                    import types
                    instance_check = type(self.points) not in (types.ListType, types.TupleType)
...
#                raise TFSValidationException('Empty end vector')
            ...
            if len(self) == 4:
                cpIntersection = calculateIntersectPoint(*self.points)
...
#        print '\t', 'left', right.description(), right.startT, right.endT
    ...
    def findIntersection_naive(self, other_segment, debugMode=False):
        point = calculateIntersectPoint(self.startPoint(),
                                                        self.endPoint(),
