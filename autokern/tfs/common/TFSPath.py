        ...
        try:
            if not onJython:
                ...
                if sys.version_info >= (3, 0): 
                    ...
                    closed_check = not isinstance(self.closed, (bool))
                    segment_check = not isinstance(self.segments, (list,tuple))

                else:

                    import types
                    closed_check = (type(self.closed) != types.BooleanType)
                    segment_check = (type(self.segments) not in (types.ListType, types.TupleType))
...
def splitLineWithLine(l0, l1):
    ...
    intersect = calculateIntersectPoint(p00, p01, p10, p11)
